...
class HttpServer:
    def __init__(self, host = '0.0.0.0', port = 2, qq_mod = False, static_dir: str = None):
        if not static_dir or not os.path.exists(static_dir) or not os.listdir(static_dir):
            raise FileNotFoundError(
                f"前端资源目录不存在或为空。路径: {static_dir}\n"
                f"请确保已在 PC 上运行 _download_web.py，并已运行 copy_to_android.py。"
            )
        
        # 记录静态文件根目录
        self.static_folder = static_dir

        self.web = Blueprint('web', __name__, static_folder=static_dir)
...
data: {ret}\n\n'''
        ...
        async def index(path):
            full_path = os.path.join(self.static_folder, path)
            
            if path and os.path.exists(full_path) and os.path.isfile(full_path):
                target_file = full_path
            else:
                # Fallback to index.html for SPA
                target_file = os.path.join(self.static_folder, 'index.html')
            
            try:
                if not os.path.exists(target_file):
                    return f"File not found: {target_file}", 404
                ...
                return await quart.make_response(content, 200, {'Content-Type': mime_type})
